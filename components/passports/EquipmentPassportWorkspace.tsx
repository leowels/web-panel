...
export default function EquipmentPassportWorkspace() {
  ...
  const renderField = (field: FieldConfig) => {
    const value = profile?.[field.section]?.[field.key]
    if (field.type === 'textarea') {
      return (
        <label key={`${field.section}.${field.key}`} className="block md:col-span-2">
    ...
    if (field.type === 'checkbox') {
      return (
        <label key={`${field.section}.${field.key}`} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
          <input
            type="checkbox"
                ...
                {activeTab === 'overview' && (
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 text-sm">
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="text-slate-500">Регистрационный номер</div><div className="mt-1 font-semibold text-slate-900">{passport.equipment.registration_number || '—'}</div></div>
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="text-slate-500">Заводской номер</div><div className="mt-1 font-semibold text-slate-900">{passport.equipment.factory_number || '—'}</div></div>
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="text-slate-500">Инвентарный номер</div><div className="mt-1 font-semibold text-slate-900">{passport.equipment.inventory_number || '—'}</div></div>
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="text-slate-500">Изготовитель</div><div className="mt-1 font-semibold text-slate-900">{passport.equipment.manufacturer || '—'}</div></div>
                    </div>
